...
def prepare_data():
    ...
    # bins = [0, 15, 40, 70, 130, 250, 413]
    # labels = ['Super', 'Top', 'Mid', 'Average', 'Low', 'Bottom']
    ...
    #get dummies is similar to OneHotEncoding
    mod_data_cat = pd.get_dummies(mod_data[cat_columns])
    mod_data = mod_data.drop(columns=cat_columns)
    mod_data = pd.concat([mod_data, mod_data_cat], axis=1)
    
    output_file = os.path.join("Data", "cat_to_num.csv")  
    mod_data.to_csv(output_file, index=False)
